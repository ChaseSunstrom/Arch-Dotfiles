return {
    ...
    config = function()
        ...
        local wp = require("config.wal_profiles")

        vim.opt.termguicolors = true
        vim.api.nvim_create_autocmd("ColorScheme", { callback = function() pcall(wp.reapply_alpha_highlights) end })
        vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function()
                vim.wo.winhighlight = table.concat({
                    ...
                    "EndOfBuffer:AlphaEndOfBuffer",
                }, ",")
            end,
        })

        local function getCharLen(s, pos)
            local byte = string.byte(s, pos); if not byte then return nil end
        ...
        local function applyColors(logo, colors, logoColors)
            ...
            for key, color in pairs(colors) do
                local name = "Alpha" .. key; vim.api.nvim_set_hl(0, name, color); colors[key] = name
            end
            dashboard.section.header.opts.hl = {}
            for i, line in ipairs(logoColors) do
                local highlights, pos = {}, 0
                for _ = 1, #line do
                    local opos = pos; pos = pos + getCharLen(logo[i], opos + 1)
                    local color_name = colors[line:sub(_, _)]
